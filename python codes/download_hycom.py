...
# Further modify by Gabriel Coutinho (gabrielsantincoutinho@gmail.com) for standalone use also (https://github.com/HollowGbrl)
# ----------
...
var_list = 'ssh,steric_ssh,u_barotropic_velocity,v_barotropic_velocity'
...
north = -26              # -80 to 80 degN          
south = -34              # -80 to 80 degN
west = -49 + 180         # 0 to 360 degE
east = -46 + 180         # 0 to 360 degE

# - - -
# specify the directory where the extracted nc files will be saved:
resultDirectory = './dados_hycom/'         # include the ending '/'
...
date_start = '2020-01-10 00:00:00'      # ISO formatted string for the starting datetime for the data to be downloaded (starting hour must be either 00, 03, 06, 09, 12, 15, 18, or 21, and must be 12 if the date_start is Jan 1 of the year or first day of the expt. The date_start must be within the range of dates for the glb and expt as described at www.hycom.org)
number_of_days  = 800                   # number of days of 1-hourly data to be downloaded (note: all downloaded days must fall within the range for the glb and expt as described at www.hycom.org)
...
def get_extraction(dt, out_fn, var_list):
    ...
    while (counter <= 10) and (got_file == False):
        print(f'  Attempting to get data, number of attempts = {counter}')
        tt0 = time.time()
        try:
